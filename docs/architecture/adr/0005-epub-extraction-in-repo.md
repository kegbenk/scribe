...
## Addendum (0.4.x) — cover-image extraction

The EPUB path now surfaces the book's declared cover art as an **additive** optional `metadata.coverImage` (a base64 data URI) on the `contentStructure` output, so consumers (velo-macos's library grid) can render real cover art instead of a gradient. Discovery order: (a) EPUB3 manifest item with `properties` containing `cover-image`; (b) EPUB2 `<meta name="cover" content="ITEM_ID"/>` → manifest item by id; (c) fallback: a manifest image item whose id or href contains "cover". Loaded via `ZipReader` and capped at the same 500KB payload limit as inline images — oversize covers are skipped (not resized), a deliberate limitation. The PDF path emits nothing (page-1 rasterization is a separate problem, out of scope). Strictly additive: no existing field changed; the internal `processFileForResult` tuple gained a fourth `coverImage` element while the public `processForTest`/`extractContent` signatures are unchanged.
