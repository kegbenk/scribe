...
3. ✅ **One consumer contract exercised end-to-end at runtime** — satisfied 2026-07-02, two ways:
   ...
   - **rsvp-reader (simulator-level):** Scribe 0.3.0 (the version its pin resolved at the time) linked and run inside the iOS simulator against a real PDF, producing the exact `VeloPDFProcessor.processFile` bridge assembly (`{chapters, toc, hasStructure, images}`), schema-valid. The literal in-app Capacitor call remains blocked by rsvp-reader's device-only Realm static lib (pre-existing, unrelated to Scribe); a physical-device run or a Realm/SPM migration would close that residual.
