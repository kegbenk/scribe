...
- [x] **Document `DocumentIntelligence` public API in README.md** — done 2026-07-02. New "Document Intelligence (optional)" section: what it is (optional enrichment per ADR-0002, never gates deterministic extraction), the two availability tiers (NaturalLanguage vs FoundationModels, both under the `@available(iOS 26/macOS 26)` gate, AI tier additionally behind `isAvailable`), and a realistic code sample using the actual public API (`extractWithNLP`, `analyze`, `DocumentAnalysis.summary/.classification/.entities`).
- [x] **Smoke-test cycle** (rsvp-reader) — done 2026-07-02. Beyond building green, Scribe 0.3.0 (the version rsvp-reader resolves) was run **inside the iOS simulator** against a real PDF and produced schema-valid, bridge-shaped `contentStructure` (10 chapters/10 toc/2 images); the EPUB path (64 chapters) was runtime-verified the same way against unreleased main. See MILESTONE_GATES gate #3 for the level achieved (Scribe-in-simulator + faithful bridge assembly + schema validation; the full in-app Capacitor call is blocked by rsvp-reader's device-only Realm CocoaPod failing to link for the simulator — unrelated to Scribe). velo-macos remains blocked by its own in-flight WIP (AuthService/SyncService not compiling — unrelated to Scribe, verified); re-run when its tree builds.
...
- [x] **EPUB cross-file chapter boundaries** — done 2026-07-02. `EPUBExtraction.swift` now builds one global paragraph stream across all spine documents (keeping per-doc image/footnote/anchor bookkeeping, rebased word offsets, and `startPage` = spine ordinal of the anchor file) and cuts chapters at global toc-anchor boundaries, so a chapter that spans a spine-file split keeps its cross-file tail instead of leaking it into the next file's first chapter. Front matter before the first anchor becomes its own chapter. pride-prejudice `chapter_titles` 0.70 → 1.00, overall 0.961 → 1.000 ("Chapter I." now ~908 words, was ~160); baseline re-locked with justification. Swedenborg now reports its 89 declared navPoints + front matter = 90 (toc-driven), not the old 93 one-per-spine-file over-count. New `testCrossFileChapterBoundaries` test + regenerated fixture cover the case.
...
- [x] **Privacy audit script** — done 2026-07-02. `tools/privacy-audit.sh` mechanically enforces `docs/ops/privacy-principles.md`'s audit checklist on `swift/Sources/Scribe`: (1) no networking APIs anywhere in the core (`URLSession`/`NSURLConnection`/`NWConnection`/`import Network`/…, flagged even in comments); (2) no `http(s)://`/`fetch` in non-comment code (doc-URL links in comments are allowed via colon-aware comment stripping); (3) no external `Package.swift` dependencies (root manifest zero; dev manifest limited to CLI-only swift-argument-parser). Exits non-zero on violation. Wired into CI as the first (grep-only, no-build) step and listed as a pre-tag step in the release checklist. Passes clean on the current tree.
